...
var Imager = module.exports = function Imager (config, storage) {
  ...
  // custom preset convenience shortcuts
  this.gm = gm;
  this.async = async;
  this.fs = fs;
...
Imager.prototype = {
  ...
  prepareUpload: function (file, filename, variant, fn) {
    ...
    var builtInPresets = ['resize', 'crop', 'resizeAndCrop', 'original'];
    for (var preset in variants) {
      if (!variants.hasOwnProperty(preset) ||
        builtInPresets.indexOf(preset) > -1 ||
        typeof variants[preset] !== "function") {
        continue;
      }

      asyncArr.push(this.buildPresetFunction(variants[preset], file, {
        name: preset
      }, filename));
    }
    ...
    if (variants.original) {
      var processFiles = function (cb) {
        ...
        self.copyOriginalFile(file, preset, filename, cb);
      };
      asyncArr.push(processFiles);
    }
  ...
  buildPresetFunction: function(fn, file, preset, fname) {
    var self = this;
    return function(cb) {
      fn.call(self, file, preset, fname, cb);
    }
  },
  ...
  copyOriginalFile: function (file, preset, fname, cb) {
    var filename = fname + contentType[file['type']];
    var self = this;
    var remoteFile = preset.name + '_' + filename;
    var dest = tempDir + remoteFile;

    copyFile(file['path'], dest, function(err) {
      if (err) return cb(err);
      self['pushTo' + self.storage](dest, remoteFile, filename, file['type'], cb);
    });
  },
  ...
  pushToS3: function (file, remoteFile, filename, type, cb) {
    ...
    if (this.config['storage'][this.storage]['storageClass']) {
      headers['x-amz-storage-class'] = this.config['storage'][this.storage]['storageClass'];
    }

    client.putFile(file, remoteFile, headers, function (err, res) {
      if (err) return cb(err);
      log(remoteFile + ' uploaded');
      // remove the file after successful upload
      fs.unlink(tempDir + remoteFile);
  ...
  prepareRemove: function (file, fn, variant) {
    ...
    if (variants.original) {
      asyncArr.push(function (cb) {
        self['removeFrom' + self.storage](file, 'original', cb);
      });
    }
